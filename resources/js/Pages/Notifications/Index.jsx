...
export default function NotificationsIndex({ auth, notifications }) {
    // Defensive: fallback to empty paginator structure if notifications is null/undefined
    const safeNotifications =
        notifications && notifications.data && notifications.links
            ? notifications
            : { data: [], links: [] };
    const [notificationList, setNotificationList] = useState(
        safeNotifications.data
    );
    ...
    const deleteNotification = async (notificationId) => {
        if (confirm("Are you sure you want to delete this notification?")) {
            try {
                ...
                setNotificationList(
                    notificationList.filter((n) => n.id !== notificationId)
                );
            } catch (error) {
        ...
        <MainLayout auth={auth}>
            ...
            <div className="py-12">
                <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    ...
                    {/* Pagination */}
                    {safeNotifications.links &&
                        safeNotifications.links.length > 3 && (
                            <div className="mt-6 flex justify-center">
                                <nav className="flex space-x-2">
                                    {safeNotifications.links.map(
                                        (link, index) =>
                                            link.url ? (
                                                <Link
                                                    key={index}
